...
public class NameNodeHA {
  ...
  // 当前上线的节点名称
  private static final String NAMENODE_HOST = "hadoop01";
  private static final String STANBY_HOST = "hadoop02";
  ...
  public static void main(String[] args) throws Exception 
  {
        ...
        if (path.equals(ACTIVE_PARENT) && type == EventType.NodeChildrenChanged) 
        {
          try 
          {
            ...
            if (onlyAtiveNM.size() == 0) 
            {
              ...
              {
                 zookeeper.create(LOCK_ZNODE, NAMENODE_HOST.getBytes(), Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
              }
        ...
        else if (path.equals(LOCK_ZNODE) && type == EventType.NodeCreated) 
        {
          ...
          if (NAMENODE_HOST.equals(namenode_lock_znode)) 
          {
            ...
            try
            {
              ...
              zookeeper.create(createPath, NAMENODE_HOST.getBytes(),Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
    ...
    if (activeNM.size() == 1) 
    {
      ...
      zookeeper.create(standByPath, NAMENODE_HOST.getBytes(),Ids.OPEN_ACL_UNSAFE, CreateMode.EPHEMERAL);
